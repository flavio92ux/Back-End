...
CREATE PROCEDURE verificaClienteAtivoPorEmail(IN eMail VARCHAR(100))
BEGIN
  SELECT
    IF(active = 1, 'Ativo', 'Não ativo') AS 'Status'
  FROM
    staff
  WHERE email = eMail;
END $$
...
CALL verificaClienteAtivoPorEmail('Mike.Hillyer@sakilastaff.com');
